...
// Periksa apakah process_id dan npk diterima melalui POST
if (isset($_POST['process_id']) && isset($_POST['npk'])) {
    ...
    $query = "
        SELECT 
            NULL AS old_value, 
            q.jadwal_training_process AS jadwal_training,
        ...
        -- Ambil riwayat update dari qualifications_history
        SELECT 
            qh.old_value AS old_value, 
            qh.jadwal_training_process AS jadwal_training,
            qh.created_at AS created_at,
            1 AS order_priority -- Data history diberi prioritas 1 (lebih rendah)
        FROM qualifications_history qh
        WHERE qh.process_id = '$process_id' 
            AND qh.npk = '$npk'
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<p>Tidak ada riwayat skill untuk PROCESS ini.</p>";
    }
} else {
