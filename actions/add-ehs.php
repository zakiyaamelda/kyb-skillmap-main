...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $ehs_name = trim($_POST['ehs_name']);
    $workstation_id = trim($_POST['workstation_id']);
    $min_skill = trim($_POST['min_skill']);
    $npk = trim($_POST['npk']); 

    // Validasi input tidak boleh kosong
    if (empty($ehs_name) || empty($workstation_id) || empty($min_skill) || empty($npk)) {
        die("Error: Semua input harus diisi!");
    }

    // Gunakan Prepared Statement untuk keamanan
    $sql = "INSERT INTO ehs (name, workstation_id, min_skill) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sii", $ehs_name, $workstation_id, $min_skill);
