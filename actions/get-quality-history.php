...
// Periksa apakah quality_id dan npk diterima melalui POST
if (isset($_POST['quality_id']) && isset($_POST['npk'])) {
    ...
    $query = "
        SELECT 
            NULL AS old_value, 
            q.jadwal_training_quality AS jadwal_training,
            q.created_at AS created_at,
            0 AS order_priority -- Data awal diberi prioritas 0 (lebih tinggi)
        FROM qualifications_quality q
        WHERE q.quality_id = '$quality_id' 
            AND q.npk = '$npk'
        
        UNION ALL 
        
        -- Ambil riwayat update dari qualifications_quality
        SELECT 
            qh.old_value AS old_value, 
            qh.jadwal_training_quality AS jadwal_training,
            qh.created_at AS created_at,
            1 AS order_priority -- Data history diberi prioritas 1 (lebih rendah)
        FROM qualifications_quality_history qh
        WHERE qh.quality_id = '$quality_id' 
            AND qh.npk = '$npk'
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<p>Tidak ada riwayat skill untuk QUALITY ini.</p>";
    }
} else {
